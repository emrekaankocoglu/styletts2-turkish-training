...
class Trainer(object):
    ...
    def _train_epoch(self):
        ...
        for train_steps_per_epoch, batch in enumerate(tqdm(self.train_dataloader, desc="[train]"), 1):
            ...
            # Log every step
            if train_steps_per_epoch % 10 == 0 or train_steps_per_epoch == 1:
                self.logger.info(f"step {train_steps_per_epoch}: loss={losses['loss']:.4f}, ctc={losses['ctc']:.4f}, s2s={losses['s2s']:.4f}")
